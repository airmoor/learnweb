...
def get_site_sections(context):
    return context['request'].site.root_page.get_children().live()
...
def is_section(page, current_page):
    # sections = Page.objects.child_of(homepage)

    sections = homepage.get_children()
    return (True if current_page in sections else False)

def sections(page, current_page):
